...
`bet` is the shell alias `setup.sh` offers to install; it expands to
`cd <repo> && source .venv/bin/activate && python3 -m betedge`. Without
the alias, run `python3 -m betedge …` from the repo with the virtualenv
active. There is no `betedge` executable on PATH — `[project.scripts]` in
`pyproject.toml` declares one, but nothing installs the package, so that
entry point is never created.
...
| `bet rt discover` | free | Find RT markets on Kalshi and write the contracts file, rules text included |
